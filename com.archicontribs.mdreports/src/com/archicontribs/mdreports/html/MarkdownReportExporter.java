...
public class MarkdownReportExporter {
    ...
    public static File PREVIEW_FOLDER = new File(ArchiPlugin.INSTANCE.getWorkspaceFolder(), "markdown-report-preview"); //$NON-NLS-1$
    ...
    public void export() throws Exception {
        ...
        IRunnableWithProgress runnable = monitor -> {
            try {
                file[0] = createReport(targetFolder, "index.markdown", monitor); //$NON-NLS-1$
            }
        ...
        if(exception[0] instanceof CancelledException) {
            MessageDialog.openInformation(Display.getCurrent().getActiveShell(), Messages.MarkdownReportExporter_2, exception[0].getMessage());
            return;
        }
    ...
    public void preview() throws Exception {
        ...
        IRunnableWithProgress runnable = monitor -> {
            try {
                file[0] = createReport(PREVIEW_FOLDER, "preview-" + fModel.getId() + ".markdown", monitor);  //$NON-NLS-1$//$NON-NLS-2$
            }
            catch(Exception ex) {  // Catch OOM and SWT exceptions
                exception[0] = ex;
            }
        ...
        IBrowserEditorInput input = new BrowserEditorInput("file:///" + file[0].getPath(), Messages.MarkdownReportExporter_0 + " " + fModel.getName()); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public File createReport(File targetFolder, String indexFileName, IProgressMonitor monitor) throws IOException {
        ...
        File mainFile = new File(ArchiMarkdownReportsPlugin.INSTANCE.getTemplatesFolder(), "st/main.stg"); //$NON-NLS-1$
        ...
        writeElement(new File(elementsFolder, "model.markdown"), stFrame, fModel); //$NON-NLS-1$
        ...
        // Write root model.markdown frame
    ...
    /**
     * Copy source Markdown files to target folder
     * @throws IOException 
     */
    private void copyMarkdownSkeleton(File targetFolder) throws IOException {
        setProgressSubTask(Messages.MarkdownReportExporter_9);
        
        File srcDir = new File(ArchiMarkdownReportsPlugin.INSTANCE.getTemplatesFolder(), "markdown"); //$NON-NLS-1$
    ...
    private void writeElements(File elementsFolder, ST stFrame, List<EObject> list) throws IOException {
        for(EObject object : list) {
            if(object instanceof IArchimateConcept) {
                writeElement(new File(elementsFolder, ((IIdentifier) object).getId() + ".markdown"), stFrame, object); //$NON-NLS-1$
            }
        }
    }
        ...
        for(IDiagramModel dm : fModel.getDiagramModels()) {
            for(Iterator<EObject> iter =  dm.eAllContents(); iter.hasNext();) {
                ...
                if(eObject instanceof IDiagramModelObject && !(eObject instanceof IDiagramModelArchimateObject) 
                    ...
                    writeElement(new File(objectsFolder, ((IIdentifier) eObject).getId() + ".markdown"), stFrame, eObject); //$NON-NLS-1$
                }
            }
        }
    }
    ...
    private void writeDiagrams(File imagesFolder, File viewsFolder, ST stFrame) throws IOException {
        ...
        for(IDiagramModel dm : diagramModels) {
            ...
            File viewFile = new File(viewsFolder, dm.getId() + ".markdown"); //$NON-NLS-1$
