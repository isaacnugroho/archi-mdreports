 ...
 */
package com.archicontribs.mdreports.markdown;
...
import com.archicontribs.mdreports.ArchiMarkdownReportsPlugin;

/**
 * Export model to Markdown report
 * 
...
public class MarkdownReportExporter {
  ...
  public static File PREVIEW_FOLDER = new File(ArchiPlugin.INSTANCE.getWorkspaceFolder(),
      "markdown-report-preview"); //$NON-NLS-1$
  ...
  public MarkdownReportExporter(IArchimateModel model) {
    fModel = model;
  }
  ...
  public void export() throws Exception {
    ...
    IRunnableWithProgress runnable = monitor -> {
      try {
        file[0] = createReport(targetFolder, "index.md", monitor); //$NON-NLS-1$
    ...
    if (exception[0] instanceof CancelledException) {
      MessageDialog.openInformation(Display.getCurrent().getActiveShell(),
          Messages.MarkdownReportExporter_2, exception[0].getMessage());
      return;
...
//        browser.openURL(new URL("file", null, file[0].getAbsolutePath().replace(" ", "%20"))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  public void preview() throws Exception {
    ...
    IRunnableWithProgress runnable = monitor -> {
      try {
        file[0] = createReport(PREVIEW_FOLDER, "preview-" + fModel.getId() + ".md", monitor);  //$NON-NLS-1$//$NON-NLS-2$
      } catch (Exception ex) {  // Catch OOM and SWT exceptions
        exception[0] = ex;
      }
    };
    ...
    IBrowserEditorInput input = new BrowserEditorInput("file:///" + file[0].getPath(), //$NON-NLS-1$
        Messages.MarkdownReportExporter_0 + " " + fModel.getName());  //$NON-NLS-1$
  ...
  public File createReport(File targetFolder, String indexFileName,
      IProgressMonitor monitor) throws IOException {
    progressMonitor = monitor;

    if (progressMonitor != null) {
      progressMonitor.beginTask(Messages.MarkdownReportExporter_6, -1);
    }

    // Copy Markdown skeleton to target
//        copyMarkdownSkeleton(targetFolder);

    // Copy hints files from the help plug-in
    copyHintsFiles(targetFolder);

    setProgressSubTask(Messages.MarkdownReportExporter_11);
    ...
    File mainFile = new File(ArchiMarkdownReportsPlugin.INSTANCE.getTemplatesFolder(),
    ...
    // Write model purpose and properties markdown
    writeElement(new File(elementsFolder, "model.md"), stFrame, fModel); //$NON-NLS-1$
    ...
    setProgressSubTask(Messages.MarkdownReportExporter_13);

    // Write root model.md frame
    ...
    try (OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(indexFile),
        "UTF8")) { //$NON-NLS-1$
  ...
  private void copyHintsFiles(File targetFolder) throws IOException {
    setProgressSubTask(Messages.MarkdownReportExporter_10);
   ...
   * Write all folders
   */
  private void writeFolders(File elementsFolder, ST stFrame,
      List<IFolder> folders) throws IOException {
   ...
   * Write all elements
   */
  private void writeElements(File elementsFolder, ST stFrame,
      List<EObject> list) throws IOException {
    for (EObject object : list) {
      if (object instanceof IArchimateConcept) {
        writeElement(new File(elementsFolder, ((IIdentifier) object).getId() + ".md"), stFrame, //$NON-NLS-1$
            object);
      }
    }
  }
  ...
  private void writeElement(File elementFile, ST stFrame, EObject component) throws IOException {
    ...
    stFrame.remove("models"); //$NON-NLS-1$
    if (component instanceof IArchimateConcept) {
      List<IArchimateDiagramModel> models = findReferencedDiagramsForArchimateConcept(
          (IArchimateConcept) component);
      stFrame.add("models", models); //$NON-NLS-1$
    }
    try (OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(elementFile),
        "UTF8")) { //$NON-NLS-1$
  ...
  /**
   * Find all diagram models that a given Archimate concept is currently referenced in (i.e it
   * appears as a graphical entity).
   * 
   * @param archimateConcept The concept to check on.
   * @return A List of diagram models that archimateConcept is currently referenced in as a node or
   *         connection. May be empty, but never null.
   */
  private static List<IArchimateDiagramModel> findReferencedDiagramsForArchimateConcept(
      IArchimateConcept archimateConcept) {
    List<IArchimateDiagramModel> models = new ArrayList<>();

    if (archimateConcept != null && archimateConcept.getArchimateModel() != null) {
      for (IDiagramModelArchimateComponent dmc : archimateConcept
          .getReferencingDiagramComponents()) {
        final IDiagramModel diagramModel = dmc.getDiagramModel();
        if (diagramModel instanceof IArchimateDiagramModel) {
          IArchimateDiagramModel model = (IArchimateDiagramModel) dmc.getDiagramModel();
    ...
    for (IDiagramModel dm : fModel.getDiagramModels()) {
      for (Iterator<EObject> iter = dm.eAllContents(); iter.hasNext();) {
        ...
        if (eObject instanceof IDiagramModelObject
          ...
          writeElement(new File(objectsFolder, ((IIdentifier) eObject).getId() + ".md"), stFrame, //$NON-NLS-1$
              eObject);
        }
      }
    }
  ...
  private void writeDiagrams(File imagesFolder, File viewsFolder, ST stFrame) throws IOException {
    ...
    for (IDiagramModel dm : diagramModels) {
      ...
      File viewFile = new File(viewsFolder, dm.getId() + ".md"); //$NON-NLS-1$
      try (OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(viewFile),
          "UTF8")) { //$NON-NLS-1$
        writer.write(stFrame.render());
      }
  ...
  private void saveImages(File imagesFolder, List<IDiagramModel> diagramModels) throws IOException {
    ...
    for (IDiagramModel dm : diagramModels) {
      setProgressSubTask(NLS.bind(Messages.MarkdownReportExporter_4, i++, total));
      ...
      try {
        ...
        if (StringUtils.isSet(diagramName)) {
          ...
          diagramName = Messages.MarkdownReportExporter_1 + " " + nameCount++ + ".png";  //$NON-NLS-1$//$NON-NLS-2$
        }
        ...
        loader.data = new ImageData[]{geoImage[0].getImage().getImageData(ImageFactory
            .getImageDeviceZoom())};
  ...
  private void checkProgressCancelled() throws CancelledException {
    if (progressMonitor != null && progressMonitor.isCanceled()) {
      throw new CancelledException(Messages.MarkdownReportExporter_14);
    }
  }
  ...
  private File askSaveFolder() {
    ...
    dialog.setText(Messages.MarkdownReportExporter_2);
    dialog.setMessage(Messages.MarkdownReportExporter_3);

    IPreferenceStore store = ArchiMarkdownReportsPlugin.INSTANCE.getPreferenceStore();
    ...
    if (folder.exists()) {
      ...
      if (children != null && children.length > 0) {
        boolean result = MessageDialog.openQuestion(Display.getCurrent().getActiveShell(),
            Messages.MarkdownReportExporter_2,
            NLS.bind(Messages.MarkdownReportExporter_5, folder));
        if (!result) {
          return null;
        }
      }
  ...
  private void addNewBounds(IDiagramModelObject dmo, int offsetX, int offsetY) {
    ...
    BoundsWithAbsolutePosition newBounds = new BoundsWithAbsolutePosition(dmo.getBounds(),
        ImageFactory.getImageDeviceZoom() / 100);
